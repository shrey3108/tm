...
export function DataTable<TData, TValue>({
  ...
  return (
    <div className="w-full space-y-1 sm:space-y-2">
      {searchKey && (
        <div className="flex w-full items-stretch justify-between gap-2 sm:flex-row sm:items-center p-2 bg-muted/20 rounded-2xl border border-muted-foreground/10 mb-4">
              ...
              <TableRow key={headerGroup.id}>
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead
                      key={header.id}
                      className="px-1 font-semibold text-base bg-muted whitespace-normal"
            ...
            {loading && data.length === 0 ? (
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  className={loading ? "opacity-50 pointer-events-none transition-opacity" : "even:bg-muted/60 hover:bg-muted-foreground/15 dark:hover:bg-muted-foreground/20"}
                >
